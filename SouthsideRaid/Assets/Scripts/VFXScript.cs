    ...
    public void SpawnCritFX()
    {
        ...
        switch (randVal)
        {
            case 1:
                ...
                VFX.transform.position = new Vector3(gameObject.transform.position.x + randXPos, gameObject.transform.position.y - 3 + randYPos, gameObject.transform.position.z);
                StartCoroutine(waitToDestroy(VFX));
                break;
            case 2:
                GameObject VFX2 = Instantiate(crit2, Vector3.zero, Quaternion.identity, gameObject.transform);
                VFX2.transform.position = new Vector3(gameObject.transform.position.x + randXPos, gameObject.transform.position.y - 3 + randYPos, gameObject.transform.position.z);
                StartCoroutine(waitToDestroy(VFX2));
                break;
            case 3:
                GameObject VFX3 = Instantiate(crit3, Vector3.zero, Quaternion.identity, gameObject.transform);
                VFX3.transform.position = new Vector3(gameObject.transform.position.x + randXPos, gameObject.transform.position.y - 3 + randYPos, gameObject.transform.position.z);
